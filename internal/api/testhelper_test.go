...
// startedTournament returns an organizer + tournament that has been started with
// 4 confirmed players, with all round-1 results submitted (so Finish is valid).
// It uses a unique per-test-name organizer/player set so it is safe to call
// multiple times within a single test (Postgres test DB is cleaned per test).
func startedTournament(t *testing.T, database *sql.DB) (*models.User, *models.Tournament) {
	t.Helper()
	ctx := context.Background()
	owner := mustCreateUser(t, database, "owner-"+t.Name()+"@example.com", "Owner-"+t.Name())
	numRounds := 2
	tourn := &models.Tournament{
		Name:        "Started " + t.Name(),
		MaxPlayers:  16,
	...
	if err := engine.WithTournamentEngine(ctx, database, tourn.ID,
		func(tx *sql.Tx, tm *models.Tournament, eng *swisstools.Tournament) (string, error) {
			...
			*eng = newEng
			// Submit all results in round 1 so Finish/NextRound is valid
...
// requestWithUser builds an *http.Request with the given user in the context
// and the given chi URL parameters resolved (so chi.URLParam works).
func requestWithUser(method, target string, body string, user *models.User, params map[string]string) *http.Request {
	if target == "" {
		target = "/"
	}
